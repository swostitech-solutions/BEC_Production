...
const AdmAttendanceEntry = () => {
  ...
  const handleUpdateClick = async () => {
    ...
    //   formDataToSend.append("assignment_file_url", existingFileUrl);
    // }
  ...
  const handleDelete = async (assignmentId) => {
    ...
    try {
      ...
      if (result.message?.toLowerCase().includes("success")) {
        ...
        // ✅ Instantly remove from table — no reload
        // setData((prevData) => {
        //   const updatedList = prevData.filter((item) => item.id !== assignmentId);
                      ...
                      <div className="col-12 col-md-3 mb-1">
                        ...
                        {/* <Select
                          ...
                          options={
                            LectureList?.map((p) => ({
                              value: p.id,
                              label: `${p.lecture_period_name} (${p.time_from} - ${p.time_to})`,
                            })) || []
                          }
                          value={
                            LectureList?.find((p) => p.id === Number(formData.period_id))
                              ? {
                                value: formData.period_id,
                                label: `${LectureList.find(
                                  (p) => p.id === Number(formData.period_id)
