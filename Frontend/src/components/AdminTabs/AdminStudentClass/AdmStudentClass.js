...
const AdmAttendanceEntry = () => {
  ...
  const handleEditClick = async (student) => {
    try {
      ...
      if (result.message === "Success" && result.data) {
        ...
        // 🟢 1. Student main details (ONLY USED IN MODAL)
        setSelectedStudent({
        ...
        // setFormData((prev) => ({
        //   ...prev,
        //   choice_month: selectedSemesterIds
        // }));
        ...
        // 🟢 5. Previous Year Fees
        // try {
        //   const pyUrl = `${ApiUrl.apiurl}StudentCourse/GetPreviousYearFees/?organization_id=${organizationId}&branch_id=${branchId}&student_id=${s.student_id}&semester_id=${s.semester_id}`;
        ...
        //   setPreviousYearBalance(
        //     pyData.message === "success!!" ? pyData.data.total_remaining_amount : 0
        //   );
        // } catch {
  ...
  //       choice_semester: isTransportAvailed
  //         ? Object.keys(selectedMonths)
  //           .filter((m) => selectedMonths[m])
  //           .map(Number)
  //         : [],
  //       // choice_semester: [Number(selectedSemester)],
  //       // choice_semester: [apiStudentIds.semester_id],
  ...
  const handleSaveStudentCourse = async () => {
    try {
      ...
      // ✅ ADD choice_semester ONLY WHEN REQUIRED
      //if (
      // isTransportAvailed &&
      //  newlySelectedSemesters.length > 0 &&
      //  !lockAllSemesters
      //) {
      // payload.choice_semester = newlySelectedSemesters;
      //}
      if (isTransportAvailed && !lockAllSemesters) {
