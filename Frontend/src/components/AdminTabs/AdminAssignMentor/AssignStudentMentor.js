...
const AssignStudentMentor = () => {
      ...
      try {
        ...
        if (fetched.length > 0) {
          setStudents((prev) => {
            ...
            const newStudents = fetched.filter(
              (s) => {
  ...
  const handleSelectStudent = (selectedStudentsFromModal) => {
    ...
    setStudents((prev) => {
      ...
      const newStudents = studentsArray.filter((s) => {
        const sid = s.studentBasicDetails?.id || s.student_id || s.id;
        return sid && !existingIds.has(sid);
      });
  ...
  // Search student by Registration No and/or School BarCode and show in the table
  const handleSearch = async () => {
    ...
    try {
      ...
      if (!organization_id || !branch_id) {
        alert("Missing organization_id / branch_id. Please login again.");
        return;
      }

      // Same API used elsewhere in this file; we fetch the list then filter locally.
      const params = new URLSearchParams({
      ...
      if (data.message === "success!!" && Array.isArray(data.data)) {
        const regNeedle = String(registrationNo || "").trim().toLowerCase();
        const barcodeNeedle = String(barcode || "").trim().toLowerCase();

        const matches = data.data.filter((s) => {
          const regHay = [
            s.registration_no,
            s.enrollment_no,
            s.college_admission_no,
          ...
          const barcodeHay = [s.barcode]
            .filter(Boolean)
            .map((v) => String(v).trim().toLowerCase());

          const regOk = regNeedle ? regHay.includes(regNeedle) : true;
          const barOk = barcodeNeedle ? barcodeHay.includes(barcodeNeedle) : true;
        ...
        const normalized = matches
          .map((s) => ({
            ...
            registration_no:
              ...
              s.school_admission_no,
            barcode: s.barcode,
            classname: s.classname || s.class_name || s.course_name,
        ...
        setStudents(normalized); // show only the matched student(s)
        setStudentName(`${normalized.length} student(s) found`);
      } else {
        alert("Student not found.");
        setStudents([]);
        setStudentName("");
      ...
      if (result.message === "Already Exists") {
        ...
        setStudents((prevStudents) =>
          prevStudents.map((student) => ({
            ...student,
            assignedMentor: selectedMentor.label,
          }))
        );
      } else {
