...
//           `${ApiUrl.apiurl}BANK/GetAllBankList/`
//         );
...
//   // Fetch data based on selectedStudent identifiers
...
//     return feeDetails.reduce((acc, item) => {
//       const key = `${item.semester}-${item.element_name}`;

//       if (!acc[key]) {
//         acc[key] = {
//           id: item.id,
//           period: item.semester, // semester is the period
//           period_id: item.semester_id, // save id
//           element_name: item.element_name,
//           element_amount: parseFloat(item.element_amount) || 0,
//           paid_amount: parseFloat(item.paid_amount) || 0,
//         };
//       } else {
//         acc[key].element_amount += parseFloat(item.element_amount) || 0;
//         acc[key].paid_amount += parseFloat(item.paid_amount) || 0;
//       }

//       acc[key].balance = (
//         acc[key].element_amount - acc[key].paid_amount
//       ).toFixed(2);
...
//   useEffect(() => {
//     // Ensure aggregatedFeeDetails is initialized once feeDetails is available
//     if (feeDetails && feeDetails.length > 0) {
//       setAggregatedFeeDetails(Object.values(aggregateFeeDetails(feeDetails)));
//     }
//   }, [feeDetails]);
...
//       const elementAmount = parseFloat(detail.element_amount) || 0;
//       const paidAmount = parseFloat(detail.paid_amount) || 0;
...
//                         {uniquePeriods
//                           .filter(
//                             (row) => row.paidAmount >= row.totalAmount / 2
//                           ) // Include fully paid and half-paid periods
//                           .map((filteredRow, index) => (
//                             <React.Fragment key={index}>
//                               {aggregatedFeeDetails
//                                 .filter(
//                                   (detail) =>
//                                     detail.period_month === filteredRow.period
//                                 )
//                                 .map((item, subIndex) => (
//                                   <tr key={subIndex}>
//                                     <td>{filteredRow.period}</td>
//                                     <td>{item.element_name}</td>
//                                     <td>{item.element_amount}</td>
