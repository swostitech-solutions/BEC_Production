  ...
  const handleSearch = () => {
    const filteredData = fullStudentData.filter((student) => {
      ...
      return (
        ...
        (!filters.admissionNo ||
          studentDetails.school_admission_no
            .toString()
            .includes(filters.admissionNo)) &&
        (!filters.barcode ||
          studentDetails.barcode.toString().includes(filters.barcode)) &&
        (!filters.schoolAdmissionNo ||
          studentDetails.admission_no
            .toString()
            .includes(filters.schoolAdmissionNo)) &&
        (!filters.classId ||
          studentDetails.addmitted_class.toString() ===
            filters.classId.toString()) &&
        (!filters.section ||
          studentDetails.addmitted_section === filters.section) &&
        (!filters.fatherName ||
