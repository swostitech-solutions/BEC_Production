...
const AdmAttendanceEntry = () => {
 ...
 const handleReceiptLinkClick = async (receiptNo) => {
   ...
   try {
     const response = await api.get(`FEESRECIPTS/GetFeeReceiptsBasedOnReceiptNo/${receiptNo}/${academicSessionId}`);
 ...
 //  This replaces your old html2pdf-based layout completely
 const generatePDF = async (data) => {
   ...
   try {
     const becLogo = await toBase64("/Assets/bec.jpeg");
   ...
   const receiptDetails = [
     ["Receipt No", data.receipt_no, "Section", data.sectionname],
     ["Receipt Date", data.receipt_date, "Father's Name", data.fathername],
     ["Student Name", data.studentname, "Fee Period", data.feeperiods],
   ...
   const paymentData = [
     [data.payment_method, data.payment_reference, data.amount.toFixed(2)],
   ];
...
const exportToPDF = async () => {
  ...
  // Add a day's data as a page to an existing PDF document
  const addDayPageToPDF = async (doc, dayData, date, paymentMethodName, isFirstPage = false) => {
    ...
    const expenses = dayData.ExpenseRecodData || [];
    const otherIncome = dayData.OtherIncomeRecordData || [];

    // Fee Receipts Section - Always show
    doc.setFont("Helvetica", "bold");
    doc.setFontSize(12);
    doc.text("FEE RECEIPTS", 15, startY);
    startY += 8;
    ...
    if (expenses.length > 0) {
      expensesTable = expenses.map((expense, index) => [
        index + 1,
        expense.expense_no || "-",
        expense.party_name || "-",
        expense.party_reference || "-",
      ...
      expensesTable = [["", "", "No data available", "", ""]];
    }
    ...
    // Other Income Section - Always show
    // Check if we need a new page
    if (startY > 250) {
      doc.addPage();
      startY = 20;
    ...
    let otherIncomeTable = [];
    if (otherIncome.length > 0) {
      otherIncomeTable = otherIncome.map((income, index) => [
        index + 1,
        income.income_no || "-",
        income.party_name || "-",
        income.party_reference || "-",
      ...
      otherIncomeTable = [["", "", "No data available", "", ""]];
    }
    ...
    // Calculation Section - Always show
    // Check if we need a new page
    if (startY > 250) {
      doc.addPage();
      startY = 20;
    ...
    const totalOutward = expenses.reduce(
      (sum, expense) => sum + parseFloat(expense.payment_amount || 0),
      0
    ) || 0;
  ...
  const exportMultipleDayBook = async () => {
    ...
    try {
      ...
      // Process each day and add as a page
      for (let i = 0; i < dates.length; i++) {
        ...
        try {
          ...
          const response = await api.get(
            "EXPENSE/DayBook/ExpenseIncomeList/",
            {
              params: {
                organization_id: orgId,
                batch_id: branchId,
          ...
          if (data && data.data) {
            // Add this day's data as a page to the PDF
            await addDayPageToPDF(doc, data.data, date, paymentMethodName, isFirstPage);
            successCount++;
          } else {
            // If API returns no data structure, still add page with empty sections
            await addDayPageToPDF(doc, {}, date, paymentMethodName, isFirstPage);
            successCount++;
          }

          isFirstPage = false; // After first page, subsequent pages are not first
        } catch (error) {
      ...
      // Save the single PDF with all pages
      if (successCount > 0) {
        const filename = `DayBook_${fromDate}_to_${toDate}.pdf`;
        doc.save(filename);
      }
      ...
      if (successCount > 0) {
        alert(
          `Export completed! Single PDF with ${successCount} page(s) generated successfully.${
            errorCount > 0 ? ` ${errorCount} day(s) failed.` : ""
          }`
        );
      } else {
        alert(
