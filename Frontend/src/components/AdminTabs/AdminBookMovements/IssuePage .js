...
const IssuePage = () => {
  ...
  const handleIssueBook = async () => {
    ...
    try {
      ...
      if (data.message === "success") {
        ...
        alert(`Error: ${data.message || JSON.stringify(data.error) || "Failed to issue books"}`);
      }
    } catch (error) {
      console.error("Error issuing books:", error);
      alert("An error occurred while issuing books.");
    }
  };
  ...
  const handleBookSelection = (selectedBook) => {
    const { id, bookCode, bookName, barcode, categoryName, subcategoryName } =
      selectedBook;
    setRows((prevRows) =>
      prevRows.map((row) =>
