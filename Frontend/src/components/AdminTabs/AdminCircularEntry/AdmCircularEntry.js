// Old Code For Circular Dropdown
...
//     const updated = {};
//     list?.forEach((item) => (updated[item.id] = newVal));
...
//       <ul
//         className="dropdown-menu p-2 shadow-sm"
//         style={{ width: "100%", maxHeight: "280px", overflowY: "auto" }}
//       >
//         {/* Search box */}
...
//                 className="me-2"
//                 onChange={() => {
//                   onItemSelect ? onItemSelect(item.id) : toggleItem(item.id);
//                 }}
//               />
//               {item.label}
//             </li>
...
const CircularEntry = () => {
  ...
  //   const batchIds = Object.keys(sessionSelections).filter(id => sessionSelections[id]);
  //   const courseIds = Object.keys(courseSelections).filter(id => courseSelections[id]);
  //   const deptIds = Object.keys(branchSelections).filter(id => branchSelections[id]);
  //   const yearIds = Object.keys(academicYearSelections).filter(id => academicYearSelections[id]);
  //   const semIds = Object.keys(semesterSelections).filter(id => semesterSelections[id]);
  //   const secIds = Object.keys(sectionSelections).filter(id => sectionSelections[id]);
  ...
  //   // Only one batch_id allowed
  //   const batchId = batchIds[0];

  //   const formData = new FormData();

  //   // IMPORTANT: API expects "[1]" not JSON.stringify("[1]")
  ...
  //     setUpdateCircularId(item.id);   // <-- THIS is the correct internal ID
  ...
  //     const smsReverseMap = { F: "Father", M: "Mother", B: "Both" };
  ...
  //   // Only attach new file if uploaded
  //   // if (uploadCircular) formData.append("circular_file", uploadCircular);

  //   // ⚠ Always include existing file if not replaced
  //   if (uploadCircular) {
  ...
  //       // fetchCircularList(); // refresh table
  //       fetchAllCirculars();
  //       resetForm();
              ...
              <div className="row g-3 circular-section-box custom-section-box ms-1">
                ...
                <div className="col-12 col-md-6 col-lg-3">
                  ...
                  >
                    ...
                    >
                      {/* <input type="checkbox" checked={selectAllSection} onChange={() => setSelectAllSection(!selectAllSection)} /> */}
                      <input
                        type="checkbox"
