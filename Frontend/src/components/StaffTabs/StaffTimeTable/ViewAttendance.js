...
const ViewAttendance = () => {
  ...
  useEffect(() => {
    console.log("🔍 Staff Teacher TT Selected values:", {
  ...
  useEffect(() => {
    const fetchTimeTableData = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          console.error(`Staff Teacher Timetable API Error: ${response.status}`);
          setTimeTableData([]);
          return;
        }

        const data = await response.json();
        console.log("Staff Teacher Timetable API Response:", data);
                                  ...
                                  timeTableData
                                    ...
                                    .map((entry, index) => (
                                      <div key={index} className="p-1 border mt-1 bg-light">
                                        ...
                                        <strong>Period:</strong> {entry.lecture}<br />
