...
function Dashboard() {
  ...
  const [upComingWorksQuarters, setUpComingWorksQuarters] = useState([]);
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= 4; i++) {
      quarters.push({
        quarter: `Quarter ${i}`,
        description: `Description for Quarter ${i}`,
        startDate: getRandomDate(),
        endDate: getRandomDate(),
      });
    }
    setUpComingWorksQuarters(quarters);
  }, []);
  ...
  // Monthly attendance data (mock data for now - replace with actual API data)
  ...
  useEffect(() => {
    ...
    const monthsToShow = months.slice(0, currentMonth + 1); // Show only up to current month
    
    const data = monthsToShow.map((month) => {
      const totalDays = Math.floor(Math.random() * 5) + 20; // 20-25 working days
      const present = Math.floor(Math.random() * 5) + (totalDays - 5); // High attendance
  ...
  useEffect(() => {
    if (monthlyAttendanceData.length > 0) {
      const totals = monthlyAttendanceData.reduce(
        (acc, month) => ({
      ...
      setAttendanceData({
        totalStudents: totals.present + totals.absent,
  ...
  const attendancePercentage = attendanceData.totalStudents
    ? Math.round((attendanceData.present / attendanceData.totalStudents) * 100)
    : 0;
                  ...
                  <Col md={9}>
                    <Row>
                      <Col md={6}>
                        ...
                        <div style={{ marginBottom: "12px", fontSize: "14px" }}>
                          <strong style={{ color: "#555", minWidth: "140px", display: "inline-block" }}>Registration No:</strong>{" "}
        ...
        <Col md={6} className="ps-2">
          ...
          >
            ...
            <div
              ...
              style={{ height: "calc(100% - 56px)", overflowY: "auto" }}
            >
              {upComingWorksQuarters && upComingWorksQuarters.length > 0 ? (
                <ListGroup variant="flush">
                  {upComingWorksQuarters.map((quarter, index) => (
                    <ListGroup.Item
                      key={index}
                      style={{
                        textAlign: "left",
                          ...
                          <div style={{ fontSize: "12px", color: "#999" }}>
                            <div className="d-flex gap-3">
                              <div>
                                ...
                                <strong>Start:</strong> {quarter.startDate}
                              </div>
                              <div>
                                <FaCheckCircle size={12} style={{ marginRight: "4px" }} />
                                <strong>End:</strong> {quarter.endDate}
                              </div>
                            </div>
                          </div>
                        </div>
