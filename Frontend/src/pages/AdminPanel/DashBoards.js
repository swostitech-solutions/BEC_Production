...
const AutoLayoutExample = () => {
  ...
  const [sessionFeeYear, setSessionFeeYear] = useState(
    new Date().getFullYear(),
  );
    ...
    const fetchDashboard = async () => {
      ...
      try {
        ...
        if (json?.message === "success" && json.data) {
          setDashboardStats({
            total_active_student_count:
              json.data.total_active_student_count ?? 0,
            total_active_teacher_count:
              json.data.total_active_teacher_count ?? 0,
            total_present_student_percentage:
              json.data.total_present_student_percentage ?? 0,
          });
          setGrievances(
            Array.isArray(json.data.grievance_details)
              ? json.data.grievance_details
              : [],
          );
          setDocuments(
  ...
  useEffect(() => {
    ...
    if (!academic_year_id || !org_id || !branch_id) {
      console.warn("Missing required data for session fee:", {
        academic_year_id,
        org_id,
        branch_id,
      });
                    ...
                    <div style={cardHeaderStyle}>
                      ...
                      >
                        ...
                        <Select
                          ...
                          value={
                            yearOptions.find(
                              (opt) => opt.value === sessionFeeYear,
                            ) || null
                          }
                          onChange={(selected) =>
                            setSessionFeeYear(selected.value)
                          }
                          options={yearOptions}
                          classNamePrefix="react-select"
                          isSearchable={false}
