...
class GrievanceCreateAPIView(CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            anonymous = serializer.validated_data.get('anonymous')
    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            if filterdata:
                ...
                if student_id:
                    filterdata = filterdata.filter(created_by=student_id)
                ...
                if from_date:
                    filterdata = filterdata.filter(created_at__date__gte=from_date)

                if to_date:
                    filterdata = filterdata.filter(created_at__date__lte= to_date)
                    ...
                    for item in filterdata:
                        ...
                        if item.anonymous:
                            ...
                            data={
                                ...
                                'organization_id':item.student_course.organization.id,
                                'organization_description': item.student_course.organization.organization_description,
                                'branch_id': item.student_course.branch.id,
                                'branch_name': item.student_course.branch.branch_name,
                                'batch_id': item.student_course.batch.id,
                                'batch_name': item.student_course.batch.batch_code,
                                'course_id': None,
                                'course_name':None,
                                'academic_year_id': item.student_course.academic_year.id,
                                'academic_year': item.student_course.academic_year.academic_year_code,
                                'barcode':None,
                                'GrievanceType': item.grievance_type.grievance_type_description,
                                'GrievancePriority': item.grievance_priority.priority_type,
                                'GrievanceSeverity': item.grievance_severity.severity_type,
                                'details': item.details,
                                'submited_date': item.created_at.strftime("%Y-%m-%d %H:%M:%S"),
                                # 'file_type':file_type,
...
class GrievanceDetailsUpdateAPIView(UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            if not action_taken:
                return Response({'message':'Please provide your response'},status=status.HTTP_400_BAD_REQUEST)
            ...
            if GrievanceInstance:

                GrievanceInstance.ActionTaken = action_taken

                GrievanceInstance.ActionTakenBy = updated_by

                GrievanceInstance.ActionTakenDateTime = timezone.now()

                GrievanceInstance.status= False
