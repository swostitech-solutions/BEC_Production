...
from Acadix.models import ExceptionTrack, Organization, Batch, AcademicYear, StudentCourse, EmployeeMaster, \
...
class BookCategoryCreateAPIView(CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            category_description = serializer.validated_data.get('category_description', '').upper() if serializer.validated_data.get('category_description') else ''
            ...
            if BookCategory.objects.filter(category_name=category_name, organization_id=organization_id, batch_id=batch_id,
...
class BookLocationCreateAPIView(CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            if BookLocation.objects.filter(book_location=book_location, organization_id=organization_id, batch_id=batch_id,
                                               is_active=True).exists():
            ...
            with transaction.atomic():
                ...
                if studentId:
                    ...
                    if previous_assigned_books >= max_book_issue_allowed:
                        return Response({'error': f'Student has reached the book issue limit ({max_book_issue_allowed} books).'},
                                        status=status.HTTP_400_BAD_REQUEST)
                ...
                if staffId:
                    ...
                    if previous_assigned_books >= max_book_issue_allowed:
                        return Response({'error': f'Teacher has reached the book issue limit ({max_book_issue_allowed} books).'},
                                        status=status.HTTP_400_BAD_REQUEST)
            ...
            for item in ReturnBookDetails:
                ...
                if studentId is not None and teacherId is None:
                    ...
                    if penalty_amount and float(penalty_amount) > 0:
                        ...
                        try:
                            matching_periods = Period.objects.get(
                                period_start_date__lte=current_date,
                                period_end_date__gte=current_date
                            )
                        except ObjectDoesNotExist:
                            return Response({"message": "No Fee Period Found"},
                                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            if flag == 'B' or flag == 'b':
                ...
                try:
                    filterdata = LibraryBooksBarcode.objects.filter(
                        book_barcode_status__in=["damage", "Damage", "lost", "Lost"]
                    )
                except:
                    filterdata = None
            ...
            if flag == 'D' or flag == 'd':
                ...
                try:
                    filterdata = LibraryBooksBarcode.objects.filter(
                        book_barcode_status__in=["damage", "DAMAGE", "Damage"]
                    )
                except:
                    filterdata = None
            ...
            if filterdata:
                ...
                for item in filterdata:
                    data = {
                        ...
                        'barcode': item.barcode,
                        'isbnNo': item.book.ISBN,
                        'category': item.book.book_category.category_name,
                        'subCategory': item.book.book_sub_category.sub_category_name,
                        'publisher': item.book.publisher,
                        'author': item.book.author,
                        'publish_year': item.book.publish_year,
                        'library_branch': item.book.library_branch_id.library_branch_name,
...
class LibraryStatisticsAPIView(ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            # For staff, use employee assignments to filter by academic year
            if academic_year_id:
                ...
                staff_employee_ids = staff_assignments_qs.values_list('employee_master_id', flat=True).distinct()
