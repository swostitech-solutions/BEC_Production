        ...
        try:
            ...
            with transaction.atomic():
                for student_course_id in student_ids:
                    ...
                    Student_Instance = StudentRegistration.objects.get(id=Student_Course_Instance.student.id, is_active=True)
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            for sid in student_ids:
                ...
                # Check if student is already assigned to ANY mentor for this academic year (active only)
                existing_assignment_any_mentor = StudentMentorAssignment.objects.filter(
                    organization=organization,
                    branch=branch,
                ...
                if existing_assignment_any_mentor:
                    # Check if it's the same mentor
                    if existing_assignment_any_mentor.mentor == mentor:
                        # Already assigned to this mentor - skip it
                        skipped_students.append({
                            'student_id': sid,
                        ...
                        # Assigned to a different mentor - cannot reassign
                        other_mentor_name = f"{existing_assignment_any_mentor.mentor.employee.first_name} {existing_assignment_any_mentor.mentor.employee.last_name}".strip()
                        skipped_students.append({
                            'student_id': sid,
                            'student_name': f"{student.first_name} {student.last_name}".strip(),
                            'reason': f'Student is already assigned to another mentor ({other_mentor_name}) for this academic year. A student can only be assigned to one mentor.'
                        })
                    continue

                # Check if there's an inactive assignment with the same combination (due to unique constraint)
                # This handles the case where assignment was soft-deleted but still exists in DB
                existing_inactive_assignment = StudentMentorAssignment.objects.filter(
                    organization=organization,
                    branch=branch,
                    academic_year=academic_year,
                    mentor=mentor,
                ...
                if existing_inactive_assignment:
                    # Reactivate the existing assignment instead of creating a new one
                    existing_inactive_assignment.is_active = True
                    existing_inactive_assignment.status = "ACTIVE"
                    existing_inactive_assignment.assignment_date = assignment_date
                    existing_inactive_assignment.remarks = ""
                    existing_inactive_assignment.updated_by = created_by
                    existing_inactive_assignment.save()
                    created_assignments.append(existing_inactive_assignment)
                else:
                ...
                return Response(
                    {
                        ...
                        "reason": "All students are already assigned to a mentor (this or another mentor) or have invalid student IDs. A student can only be assigned to one mentor."
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if skipped_students:
                ...
                response_data["message"] = f"Successfully assigned {len(created_assignments)} student(s). {len(skipped_students)} student(s) were skipped (already assigned to a mentor or invalid)."
        ...
        try:
            ...
            try:
                STUDENT_MENTOR_Record = StudentMentor.objects.filter(professor=professor_id,
                                                                      academic_year=academic_year_id, organization=organization_id,
                                                                      branch=branch_id, is_active=True)
            except:
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            STUDENT_COMMUNICATION_INSTANCE = StudentCommunication.objects.create(
                organization= Organization.objects.get(id=organization_id) if organization_id else Response({"error":"organization_id is required !!!"}, status=status.HTTP_400_BAD_REQUEST) ,
                branch=Branch.objects.get(id=branch_id) if branch_id else Response({"error":"branch_id is required !!!"}, status=status.HTTP_400_BAD_REQUEST) ,
                batch=Batch.objects.get(id=batch_id) if batch_id else Response({"error":"batch_id is required !!!"}, status=status.HTTP_400_BAD_REQUEST) ,
                course=Course.objects.get(id=course_id) if course_id else Response({"error":"course_id is required !!!"}, status=status.HTTP_400_BAD_REQUEST) ,
                department=Department.objects.get(id=department_id) if department_id else Response({"error":"department_id is required !!!"}, status=status.HTTP_400_BAD_REQUEST) ,
                academic_year=AcademicYear.objects.get(id=academic_year_id) if academic_year_id else Response({"error":"academic_id is required !!!"}, status=status.HTTP_400_BAD_REQUEST) ,
                semester=Semester.objects.get(id=semester_id) if semester_id else Response({"error":"semester_id is required !!!"}, status=status.HTTP_400_BAD_REQUEST) ,
                section=Section.objects.get(id=section_id) if section_id else Response({"error":"section_id is required !!!"}, status=status.HTTP_400_BAD_REQUEST) ,
        ...
        try:
            ...
            if STUDENT_COMMUNICATION_RECORDS:
                ...
                for item in STUDENT_COMMUNICATION_RECORDS:
                    ...
                    try:
                        Professor_Instance = EmployeeMaster.objects.get(id=item.assigned_id.professor_id, is_active=True)
                    except:
