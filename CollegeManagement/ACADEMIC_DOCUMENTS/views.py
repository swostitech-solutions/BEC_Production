...
class DocumentFileListCreateAPIView(ListCreateAPIView):
    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            # Filter by upload date (uploaded_at field)
            if from_date:
                ...
                try:
                    ...
                    pass
                qs = qs.filter(uploaded_at__gte=from_datetime)
            
            if to_date:
                to_datetime = datetime.combine(to_date + timedelta(days=1), datetime.min.time())
                try:
                    to_datetime = make_aware(to_datetime)
                except:
                    pass
                qs = qs.filter(uploaded_at__lt=to_datetime)
