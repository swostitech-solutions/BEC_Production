...
const BookForm = () => {
  ...
  const getImagePreviewSrc = (imageData) => {
    if (!imageData) return "";

    if (typeof imageData === "string") {
      const trimmedValue = imageData.trim();
      ...
      const backendBaseUrl = ApiUrl.apiurl.replace(/\/api\/?$/, "");
      return `${backendBaseUrl}${trimmedValue.startsWith("/") ? "" : "/"}${trimmedValue}`;
    }

    if (typeof imageData === "object") {
      if (typeof imageData.preview === "string" && imageData.preview.trim()) {
        return getImagePreviewSrc(imageData.preview);
      }

      if (imageData.binary_data) {
        return `data:${imageData.type || "image/jpeg"};base64,${imageData.binary_data}`;
      }

      if (imageData.path) {
        return getImagePreviewSrc(imageData.path);
      }
    }
  ...
  useEffect(() => {
    if (bookData?.bookDetails?.length > 0) {
      ...
      // Set initial front cover and back cover previews
      const frontCoverPreview = getImagePreviewSrc(firstBookDetails.front_cover);
      const backCoverPreview = getImagePreviewSrc(firstBookDetails.back_cover);

      setFrontCover(frontCoverPreview ? { preview: frontCoverPreview } : null);
      setBackCover(backCoverPreview ? { preview: backCoverPreview } : null);
      if (bookData?.bookDetails?.length > 0) {
        const firstBookDetails = bookData.bookDetails[0];
        const barcodeAutoGenerated = firstBookDetails.barcode_auto_generated;
      }
              ...
              <div className="row custom-section-box  mt-3 mb-3 mx-2">
                <Row className="mb-3 mt-3">
                  <Col>
                    ...
                    {getImagePreviewSrc(frontCover) && (
                      <Image
                        src={getImagePreviewSrc(frontCover)}
                        alt="Front Cover Preview"
                        thumbnail
                        style={{
                          marginTop: "10px",
                  ...
                  <Col>
                    ...
                    {getImagePreviewSrc(backCover) && (
                      <Image
                        src={getImagePreviewSrc(backCover)}
                        alt="Back Cover Preview"
                        thumbnail
                        style={{
                          marginTop: "10px",
